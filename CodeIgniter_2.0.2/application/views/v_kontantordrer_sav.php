...

{
	...
	echo ('<select name="item-' . $item['uid'] . '-' . FF_GROCERYBAG . '">' . $bag_quantity . '</select> Gr&oslash;ntsagspose<br>');
	if ($item['item'] == FF_FRUITBAG)
	{
	echo ('<select name="item-' . $item['uid'] . '-' . FF_FRUITBAG . '">' . $bag_quantity . '</select> Frugtpose<br>');
	} 
	echo ("<br>\n");
	$count++;
}		
...
?>
	...
	<br>
	<input type="submit" value="Registrer kontantordrer" name="kontantordre"  class="form_button"/>
	</form>
