...
{
	echo ('<td><strong>' . $division['shortname'] .'</strong></td>');
}
echo ('</tr>');
...
$uniquedays = array_column($ikkeafhentet, 1);
$t2 = $ikkeafhentet[$uniquedays[0]['pickupdate']];

		foreach ($uniquedays as $days)
		{
				if ($days['pickupdate'] <> $savdate)
				{
					echo '<tr class="'.$classes[$counter%2].'"'.">\n";
					echo '<td><strong>'. $days['pickupdate'] . '</strong></td>';
					$savdate = $days['pickupdate'];
					$t2 = $ikkeafhentet[$uniquedays[$counter]['pickupdate']];
					foreach ($divisions as $division)
					{
						$q = '';
						foreach ($t2 as $t)
						{
							if ($t['division'] == $division['uid'])
							{
								$q = $t['quant'];
							}
						}
